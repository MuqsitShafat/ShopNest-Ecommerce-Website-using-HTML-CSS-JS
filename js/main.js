...
const ShopNestWishlist = {
  ...
  add(item)  {
    const list = this.get();
    if (!list.find(i => i.id === item.id)) { list.push(item); this.save(list); }
  },
...
const ShopNestCart = {
  ...
  add(item) {
    ...
    if (existing) { existing.qty++; } else { list.push({ ...item, qty: 1 }); }
    this.save(list);
  },
  remove(id) { this.save(this.get().filter(i => i.id !== id)); },
  setQty(id, qty) {
    ...
    if (item) { item.qty = qty; if (item.qty <= 0) return this.remove(id); this.save(list); }
  },
...
const productDescriptions = {
  default: "A premium quality product from ShopNest's curated collection. Designed for modern lifestyles with attention to detail and lasting comfort."
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('keydown', e => { if (e.key === 'Escape') { closeMobileMenu(); closeQuickView(); } });
  ...
  if (searchInput && searchBar) {
    searchInput.addEventListener('focus', () => { searchBar.style.maxWidth = '380px'; });
    searchInput.addEventListener('blur',  () => { searchBar.style.maxWidth = ''; });
  ...
  if (accountBtn && accountDropdown) {
    accountBtn.addEventListener('click', e => {
      e.stopPropagation();
      if (wishlistDropdown && wishlistDropdown.classList.contains('open')) wishlistDropdown.classList.remove('open');
  ...
  if (wishlistBtn && wishlistDropdown) {
    wishlistBtn.addEventListener('click', e => {
      e.stopPropagation();
      renderWishlistDropdown();
      if (accountDropdown && accountDropdown.classList.contains('open')) accountDropdown.classList.remove('open');
  ...
  function syncWishlistHeartOnPage(id, wishlisted) {
    ...
    const btn = document.querySelector(`[data-wishlist-id="${id}"], #wish-${id.replace('prod-','').replace('sp-','')}`);
    if (!btn) {
      // Try to find via card id
      const card = document.getElementById(id);
      if (card) {
        const wishBtn = card.querySelector('.product-action-btn[aria-label="Add to wishlist"]');
  ...
  function updateHeartBtn(btn, wishlisted) {
    ...
    if (wishlisted) {
      ...
      // No background color change — only icon color
    ...
    document.querySelectorAll('.product-card').forEach(card => {
      ...
      if (wishBtn) {
        ...
        wishBtn.addEventListener('click', (e) => {
          ...
          refreshWishlistHeaderIcon();
          if (wishlistDropdown?.classList.contains('open')) renderWishlistDropdown();
        });
      }
      ...
      if (addBtn && (addBtn.textContent.includes('Add to Cart') || addBtn.classList.contains('shop-add-cart'))) {
  ...
  function openQuickView(card) {
    ...
    qvContent.querySelector('.qv-add-wishlist')?.addEventListener('click', e => {
      const btn = e.currentTarget;
      const id    = btn.dataset.id;
      const added = ShopNestWishlist.toggle({
        id, name: btn.dataset.name,
        price: parseInt(btn.dataset.price), img: btn.dataset.img
      });
      const icon = btn.querySelector('.material-icons');
      icon.textContent  = added ? 'favorite' : 'favorite_border';
      icon.style.color  = added ? 'var(--coral)' : '';
      btn.childNodes[btn.childNodes.length - 1].textContent = added ? ' Remove from Wishlist' : ' Add to Wishlist';
  ...
  if (cdH || cdM || cdS) {
    function tick() {
      if (totalSeconds <= 0) { clearInterval(tid); return; }
  ...
  if (nlForm) {
    nlForm.addEventListener('submit', e => {
      ...
      setTimeout(() => { submitBtn.textContent = 'Subscribe'; submitBtn.style.background = ''; }, 3000);
    });
  }
